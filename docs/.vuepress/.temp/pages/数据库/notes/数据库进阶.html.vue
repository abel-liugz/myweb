...
<p><img src="/images/死锁.png" alt="死锁.png"></p>
...
<li>什么是脏读：由于事务读取到了其他事务未提交、未回滚前的数据，导致读取的数据最终是不存在的，这个现象就叫做脏读。
<img src="/images/读未提交.png" alt="读未提交.png"></li>
</ul>
...
<li>什么是不可重复读：在事务A执行期间，其他事务对事务A访问的数据进行修改操作，导致事务A中前后两次读取相同的数据的结构不一致，这个现象就叫做不可重复读
<img src="/images/读已提交.png" alt="读已提交.png"></li>
</ul>
<h5 id="可重复度" tabindex="-1"><a class="header-anchor" href="#可重复度" aria-hidden="true">#</a> 可重复度</h5>
<ul>
<li>repeatable read</li>
<li>解决了不可重复读问题，产生了新的问题 -- 幻读</li>
<li>什么是幻读：在事务A访问数据期间，其他事务执行了插入操作，导致事务A前后两次读取到的数据总量不一致，这个现象就叫做幻读
<img src="/images/可重复读.png" alt="可重复读.png"></li>
</ul>
<h5 id="可串行化" tabindex="-1"><a class="header-anchor" href="#可串行化" aria-hidden="true">#</a> 可串行化</h5>
<ul>
<li>serializable</li>
<li>解决了幻读问题，实现了多事务并发执行同步效果，所以这个隔离级别的并发效率时最低下的
<img src="/images/可串行化.png" alt="可串行化.png"></li>
</ul>
...
<img src="/images/隐藏字段.png" alt="隐藏字段.png"></li>
</ul>
...
<p><img src="/images/mvcc.png" alt="mvcc.png"></p>
...
<img src="/images/索引原理.png" alt="索引原理.png"></li>
...
<p><img src="/images/聚簇索引和非聚簇索引.png" alt="聚簇索引和非聚簇索引.png"></p>
...
<img src="/images/外键约束.png" alt="外键约束.png"></li>
</ul>
</li>
</ol>
